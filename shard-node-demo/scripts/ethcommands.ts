...
async function sendTransaction(argv: any) {
    const account = new Wallet(argv.from).connect(argv.provider)
...
async function bridgeFunds(argv: any, parentChainUrl: string, chainUrl: string, inboxAddr: string) {
  ...
  if (argv.wait) {
    const shardProvider = new ethers.providers.WebSocketProvider(chainUrl);
    const account = new Wallet(argv.from).connect(shardProvider)
...
export const bridgeFundsCommand = {
  ...
  handler: async (argv: any) => {
    ...
    await bridgeFunds(argv, argv.l2url, argv.shardUrl, inboxAddr)
  },
};
...
export const createERC20Command = {
  ...
  handler: async (argv: any) => {
    ...
    if (argv.bridgeable) {
      ...
      const shardProvider = new ethers.providers.WebSocketProvider(argv.shardUrl);
      ...
      for (let i = 0; i < 60; i++) {
        if (await shardProvider.getCode(tokenL2Addr) === "0x") {
      ...
      l2provider.destroy();
      shardProvider.destroy();
      return;
    }

    // no l1-l2 token bridge, deploy token on l2 directly
    argv.provider = new ethers.providers.WebSocketProvider(argv.shardUrl);
    const deployerWallet = new Wallet(
      ethers.utils.sha256(ethers.utils.toUtf8Bytes(argv.deployer)),
      argv.provider
    );
...
export const transferERC20Command = {
  ...
  handler: async (argv: any) => {
    ...
    await(await tokenContract.transfer(new Wallet(argv.to).address, amountToTransfer)).wait();
    argv.provider.destroy();
  },
};
...
export const sendL1Command = {
  ...
  handler: async (argv: any) => {
    if (argv.to === "") {
      console.log("Please specify an address to send funds to");
      return
    }
